...
public:
    ...
    bool init(std::vector<hardware_interface::JointHandle> &joint_handles, ros::NodeHandle &nh)
    {
       ...
       // KDL::Chain chain;
        ...
        id_solver.reset(new KDL::ChainIdSolver_RNE(kdl_chain_, KDL::Vector(0, 0, -9.81)));
        ROS_INFO("Initialized kdl inverse dynamics solver");
        id_solver->updateInternalDataStructures();
        std::cout<<"nj:"<<kdl_chain_.getNrOfJoints()<<"\t" <<"ns:"<<kdl_chain_.getNrOfSegments()<<std::endl;
